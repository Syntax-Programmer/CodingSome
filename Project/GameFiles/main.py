...
def parse_config_json() -> tuple[float, float, float, Neutral.PlayerHandler]:
    ...
    player_handler = Neutral.PlayerHandler(
        config_data["player"]["initial_pos"], player_assets
    )
    return (
        config_data["gravity"],
...
def draw(
    ...
    player_handler.player_group.draw(screen)
...
def main(screen: pygame.Surface) -> None:
    ...
    GRAVITY, player_run_speed, player_jump_speed, player_handler = parse_config_json()
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key in [pygame.K_SPACE, pygame.K_UP]:
                    player_handler.trigger_jump_event(player_jump_speed)
        player_handler.player_attribute_handler(
            GRAVITY, terrain_handler, current_fps, moving_vel=player_run_speed
        )
        draw(screen, bg_image, bg_pos, player_handler, terrain_handler)
    pygame.quit()
    exit()
