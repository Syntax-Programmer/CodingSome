...
class Characters(ObjectInit, Mechanics, Health):
    x_vel = y_vel = animation_count = jump_count = frame_jumped = 0
    on_ground = True
    ...
    def __init__(
        ...
        # This presupposes that all the character in the game could be killed.
        is_killable = True
        ObjectInit.__init__(
            self,
            character_pos,
    ...
    def set_pos(self, pos_to_set: tuple[int, int]) -> None:
        #! IF we instead do self.rect.topleft the object shoots off to the moon.
    ...
    def apply_gravity(self, gravity: float, current_fps: float) -> None:
        if not self.on_ground:
            # This allows the player to perform the double jump action when falling from a ledge.
